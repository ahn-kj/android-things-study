...
public class DoorbellActivity extends Activity {
    ...
    private final static int MESSAGE_SET = 100;
    private final static int TAKE_PICTURE_DELAYED = 1000 * 5;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        weakRefHandler.sendEmptyMessage(MESSAGE_SET);
    }

    WeakRefHandler weakRefHandler = new WeakRefHandler(this);
    private class WeakRefHandler extends Handler{
        private final WeakReference<DoorbellActivity> mHandlerActivity;
        public WeakRefHandler(DoorbellActivity activity) {
            mHandlerActivity = new WeakReference<>(activity);
        }
        @Override
        public void handleMessage(Message msg) {
            super.handleMessage(msg);
            DoorbellActivity activity = mHandlerActivity.get();
            if ( activity == null ) return;
            activity.takePicture();
            weakRefHandler.sendEmptyMessageDelayed(MESSAGE_SET, TAKE_PICTURE_DELAYED);
        }
    }
    ...
    private void onPictureTaken(final byte[] imageBytes) {
        ...
        Log.d(TAG, "onPictureTaken : " + imageBytes);
        if (imageBytes != null) {
